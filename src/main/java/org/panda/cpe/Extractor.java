...
public class Extractor
{
	...
	public static void main(String[] args) throws IOException
	{
		...
		String biopaxFile = args[0];
		String blacklistFile = args[1];
		String outFile = args[2];

		if (!Files.exists(Paths.get(biopaxFile)))
		{
			System.err.println("Cannot find the BioPAX file: " + Paths.get(biopaxFile).toString());
			return;
		}

		if (!Files.exists(Paths.get(blacklistFile)))
		{
			System.err.println("Cannot find the blacklist file: " + Paths.get(blacklistFile).toString());
			return;
		}

		if (outFile.endsWith(".sif")) outFile = outFile.substring(0, outFile.length() - 4);

		SimpleIOHandler h = new SimpleIOHandler(BioPAXLevel.L3);
		Model model = h.convertFromOWL(new FileInputStream(biopaxFile));
...
//		blacklist.write(new FileOutputStream("../biopax-pattern/blacklist.txt"));
		...
		Blacklist blacklist = new Blacklist(blacklistFile);

		run(model, blacklist, outFile);
		tc.print();
	}

	public static void run(Model model, Blacklist blacklist, String outFileWoExt) throws IOException
	{
